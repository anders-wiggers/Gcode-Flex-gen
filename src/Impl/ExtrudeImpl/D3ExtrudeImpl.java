...
public class D3ExtrudeImpl implements Extrude {
    Extrude flex, soild;
    ...
    public String[] GenerateLayer(Position start, double length, double width, boolean first) {
        ...
        if ( start.getZ() > Util.getInstance().getHeight()/2 ) path.addAll(Arrays.asList(flex.GenerateLayer(start,length,width,first)));
        else path.addAll(Arrays.asList(soild.GenerateLayer(start,length,width,first)));
